use crate::constants::drivetrain::{
    SWERVE_TURN_RATIO, WHEELBASE_LENGTH_INCHES, WHEELBASE_WIDTH_INCHES,
};
use crate::constants::joystick_map::SLOW_MODE;
use crate::constants::robotmap;
use crate::constants::robotmap::drivetrain::{
    BL_DRIVE_ID, BL_ENCODER_ID, BL_TURN_ID, BR_DRIVE_ID, BR_ENCODER_ID, BR_TURN_ID, FL_DRIVE_ID,
    FL_ENCODER_ID, FL_TURN_ID, FR_DRIVE_ID, FR_ENCODER_ID, FR_TURN_ID,
};
...
impl Drivetrain {
    pub fn new() -> Self {
        Self {
            ...
            kinematics: Kinematics::new_rectangle(
                Length::new::<inch>(WHEELBASE_LENGTH_INCHES),
                Length::new::<inch>(WHEELBASE_WIDTH_INCHES),
            ),
    ...
    pub fn control_drivetrain(&mut self, joysticks: &mut Joysticks) {
        if joysticks
            .left_drive
            .get(constants::joystick_map::ZERO_HEADING)
        {
    ...
    pub fn set_speeds(&mut self, x: f64, y: f64, rotation: f64) {
        ...
        let mut speeds = self
            .kinematics
            .calculate(transform.x, transform.y, rotation);
        speeds = self.optimize(speeds);

        self.fl_drive.set(ControlMode::Percent, speeds[0].0);
        self.fl_turn.set(
            ControlMode::Position,
            speeds[0].1.get::<revolution>() * SWERVE_TURN_RATIO,
        );

        self.bl_drive.set(ControlMode::Percent, speeds[1].0);
        self.bl_turn.set(
            ControlMode::Position,
            speeds[1].1.get::<revolution>() * SWERVE_TURN_RATIO,
        );

        self.br_drive.set(ControlMode::Percent, speeds[2].0);
        self.br_turn.set(
            ControlMode::Position,
            speeds[2].1.get::<revolution>() * SWERVE_TURN_RATIO,
        );

        self.fr_drive.set(ControlMode::Percent, speeds[3].0);
        self.fr_turn.set(
            ControlMode::Position,
            speeds[3].1.get::<revolution>() * SWERVE_TURN_RATIO,
        );
    }
    ...
    pub fn get_module_angles(&self) -> Vec<Angle> {
        vec![
    ...
    pub fn optimize(&self, target_speeds: Vec<(f64, Angle)>) -> Vec<(f64, Angle)> {
        target_speeds
    }
}
